  ...
  >
    <div class="template-dialog-content">
      ...
      <div class="template-main">
        <div class="template-main" v-if="selectedTemplateId === 22">
...
export default {
  ...
  methods: {
    ...
    async selectTemplate(template) {
      ...
      // 根据选中的模板ID获取模板内容
      try {
        const templateResponse = await queryTemplateContentById({
          id: template.id,
        });
    ...
    async handleSave() {
      ...
      try {
        const res = await axios.post(`${config.DIFT_URL}/api/generate-prompt`, {
          template_name: this.selectedTemplate,
          template_content: this.templateContent,
        });
        const resData = await res.data.data.prompt?.replace(
          /<think>[\s\S]*?<\/think>\n*/,
          ""
        );
        const response = await updateTemplatePrompt({
